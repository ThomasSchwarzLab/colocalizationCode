...
for (z = 0; z < nSlices; z++) {setSlice(z+1); print("\\Update:"+(z/nSlices*100)); // updating progress
	for (x = 0; x < getWidth(); x++) {	for (y = 0; y < getHeight(); y++) { 
		...
		intensity=(intensity-min)/max*(pow(2, bitDepth())-1); // stretching pixels from 0 to max
...
for (z = 0; z < nSlices; z++) {setSlice(z+1); print("\\Update:"+(z/nSlices*100)); // updating progress
	for (x = 0; x < getWidth(); x++) { for (y = 0; y < getHeight(); y++) { 
		if (getPixel(x, y)<organelleIntensity) {setPixel(x, y, 0); // if pixel less than thrshold, then set it to zero
...
selectImage(inputImageID); rename("organelleChannel"+UID); run("RGB Color");
